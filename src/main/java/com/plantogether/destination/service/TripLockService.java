 ...
 *
 * <p>Must be called inside an active transaction — the lock is released automatically at commit /
 * rollback. Serializes concurrent writes (castVote, retractVote, upsertConfig) for the same trip,
 * enforcing the SIMPLE "one vote per trip per device" and RANKING "unique rank per device"
 * invariants that cannot be expressed as pure DB constraints (APPROVAL shares the same table with
 * rank IS NULL by design).
 */
@Service

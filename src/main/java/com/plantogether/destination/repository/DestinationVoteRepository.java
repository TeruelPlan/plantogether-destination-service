...
public interface DestinationVoteRepository extends JpaRepository<DestinationVote, UUID> {
    ...
    Optional<DestinationVote> findByTripIdAndDeviceIdAndRank(UUID tripId, UUID deviceId, Integer rank);

    @Modifying
    @Query("delete from DestinationVote v where v.destinationId = :destinationId and v.deviceId = :deviceId")
    int deleteByDestinationIdAndDeviceId(UUID destinationId, UUID deviceId);

    @Modifying
    @Query("delete from DestinationVote v where v.tripId = :tripId and v.deviceId = :deviceId and v.destinationId <> :keepDestinationId")
    int deleteOtherTripVotes(UUID tripId, UUID deviceId, UUID keepDestinationId);

    @Modifying
    @Query("update DestinationVote v set v.rank = null, v.updatedAt = current_timestamp where v.tripId = :tripId and v.deviceId = :deviceId and v.rank = :rank and v.destinationId <> :keepDestinationId")
    int clearRankForSwap(UUID tripId, UUID deviceId, Integer rank, UUID keepDestinationId);

    @Modifying
    @Query("update DestinationVote v set v.rank = null, v.updatedAt = current_timestamp where v.tripId = :tripId")
